...
To update the database use this command from the Web project folder (replace `GmailOrganizer` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../GmailOrganizer.Infrastructure/GmailOrganizer.Infrastructure.csproj -s GmailOrganizer.Web.csproj
```
